...
md("# Team Odysseus — DataSprint 2026\n## Predicting Financial Status of Kenyan Adults\n\n**Objective:** Classify Kenyan adults' financial status into 3 classes: Improved, Stayed the same, Worsened.\n\n**Dataset:** 2024 FinAccess Household Survey — 20,848 observations, 28 raw features.\n\n**Metric:** Weighted F1-Score.\n\n**Class Distribution:** Worsened 52.6% | Stayed same 26.9% | Improved 20.5%")
...
print(f"Duplicates: Found {dupes_before}, dropped. Rows now: {df.shape[0]}")
...
junk_education = ['Refused to Answer (DO NOT READ OUT)', "Don't know (DO NOT READ OUT)", 'Other (Specify)', '95']
rows_before = len(df)
df = df[~df['education_level'].isin(junk_education)]
print(f"education_level: Dropped {rows_before - len(df)} junk rows.")

# Fix marital_status
junk_marital = ["Don't know   (DO NOT READ OUT)", "Refused to Answer(DO NOT READ OUT)"]
rows_before = len(df)
df = df[~df['marital_status'].isin(junk_marital)]
print(f"marital_status: Dropped {rows_before - len(df)} junk rows.")

# Fill barriers_bank NaN (MNAR — people with bank accounts have no barrier)
missing_before = df['barriers_bank'].isnull().sum()
df['barriers_bank'] = df['barriers_bank'].fillna('No barrier')
print(f"barriers_bank: Filled {missing_before} NaN with 'No barrier'.")

# Fix barriers_mobile_money
count_zero = (df['barriers_mobile_money'] == '0').sum()
df['barriers_mobile_money'] = df['barriers_mobile_money'].replace('0', 'No barrier')
print(f"barriers_mobile_money: Renamed {count_zero} '0' values to 'No barrier'.")
...
print(f"Features: {X.shape[1]} (before target encoding)")
...
# Target distribution
df_eda['financial_status'].value_counts().plot(kind='bar', ax=axes[0], color=['#e53935', '#ff9800', '#43a047'])
axes[0].set_title('Financial Status Distribution', fontsize=14)
axes[0].set_ylabel('Count')
for label in axes[0].get_xticklabels():
    label.set_rotation(0)

# Income by class
df_eda.boxplot(column='monthly_income', by='financial_status', ax=axes[1])
axes[1].set_title('Monthly Income by Financial Status')
axes[1].set_xlabel('')

# Shock prevalence
shock_rates = df_eda.groupby('financial_status')['experienced_shock'].apply(lambda x: (x=='Yes').mean())
shock_rates.plot(kind='bar', ax=axes[2], color=['#43a047', '#ff9800', '#e53935'])
axes[2].set_title('Shock Prevalence by Financial Status', fontsize=14)
axes[2].set_ylabel('Proportion Experienced Shock')
...
print(f"  Counties: {df_eda['county'].nunique()}")
print(f"  Mobile money: {(df_eda['mobile_money_access']=='Yes').mean()*100:.1f}%")
print(f"  Experienced shock: {(df_eda['experienced_shock']=='Yes').mean()*100:.1f}%")
print(f"  Defaulted: {(df_eda['defaulted']=='Yes').mean()*100:.1f}%")
print(f"  Median income: KES {df_eda['monthly_income'].median():,.0f}")""")

md("## 5. ML Pipeline\n\n### 5a. Utility Classes & Functions")

code("""class ThresholdWrapper:
...
md("### 5b. Train/Test Split & Target Encoding")

code("""worsened_cls = list(class_names).index('Worsened')
...
print(f"{X_train_final.shape[1]} features after target encoding")""")

md("### 5c. Optuna Hyperparameter Tuning (CatBoost)")

code("""def cb_objective(trial):
    p = {
        'iterations': 300,
...
print(f"Best CatBoost: CV={study.best_value:.4f}")""")

md("### 5d. Cross-Validation (5-Fold, SMOTE-Tomek train only)")

code("""def get_base_models():
    return [
        ("XGB", XGBClassifier(
...
md("### 5e. Stacking Meta-Learner & Full Retraining")

code("""# Stacking meta-learner on OOF predictions
...
# Retrain on full train set with SMOTE-Tomek
smt = SMOTETomek(random_state=42)
...
md("### 5f. Test Scores & Blended Ensemble")
...
# Blended Ensemble - joint blend + threshold search on OOF
...
print(f"Best blend: {dict(zip(model_names_list, best_blend_w.round(3)))}")
print(f"Best thresh: {best_thresh_w.round(3)}, OOF F1: {best_blend_score:.4f}")
...
blend_f1 = f1_score(y_test, np.argmax(blend_proba * best_thresh_w, axis=1), average='weighted')
...
md("### 5g. Threshold Optimization (Individual Models)")

code("""for mn in ["CatBoost", "XGB", "LGBM", "GB"]:
    ...
    print(f"  {mn}: weights={bw.round(3)}  Test F1={tf1:.4f}")""")

md("### 5h. Final Leaderboard & Evaluation")

code("""final_lb = sorted(test_scores.items(), key=lambda x: x[1], reverse=True)
...
X_sample = X_test_s.sample(n=min(500, len(X_test_s)), random_state=42)
...
# Vulnerability Index
proba = shap_model.predict_proba(X_full_s)
worsened_idx = class_names.index('Worsened')
if best_name_s in threshold_weights_s:
    raw_worsened_prob = (proba * threshold_weights_s[best_name_s])[:, worsened_idx]
else:
...
print(f"Vulnerability: range={vulnerability.min():.1f}-{vulnerability.max():.1f}, mean={vulnerability.mean():.1f}")
print(f"High Risk (>70): {(vulnerability>70).sum()} ({(vulnerability>70).mean()*100:.1f}%)")
...
for c, r in county_stats.head(5).iterrows():
    print(f"  {c}: avg={r['avg_vulnerability']:.1f}, worsened={r['worsened_rate']*100:.0f}%")

fig, ax = plt.subplots(figsize=(14, 10))
colors = ['#d32f2f' if v > 60 else '#ff9800' if v > 50 else '#4caf50' for v in county_stats['avg_vulnerability']]
...
baseline = get_vulnerability(X_all_i)
interventions = []

# 1. Mobile Money
X_sim = X_all_i.copy()
mask = df_orig['mobile_money_access'] == 'No'
X_sim.loc[mask, 'mobile_money_access'] = 1
X_sim.loc[mask, 'digital_access'] = X_sim.loc[mask, 'mobile_ownership_1']
sim = get_vulnerability(X_sim)
interventions.append(("Mobile Money\\nfor All", baseline[mask].mean(), sim[mask].mean(), mask.sum(), (baseline[mask]>70).sum(), (sim[mask]>70).sum()))

# 2. Financial Literacy
X_sim = X_all_i.copy()
mask = df_orig['fl_score'].isin(['None correct', 'One correct'])
X_sim.loc[mask, 'fl_score'] = 3
X_sim.loc[mask, 'financial_capability'] = X_sim.loc[mask, 'education_level'] * 3
sim = get_vulnerability(X_sim)
interventions.append(("Financial Literacy\\nProgram", baseline[mask].mean(), sim[mask].mean(), mask.sum(), (baseline[mask]>70).sum(), (sim[mask]>70).sum()))

# 3. Formal Savings
X_sim = X_all_i.copy()
mask = df_orig['Savings_formal'] == 'Non-usage'
X_sim.loc[mask, 'Savings_formal'] = 1
X_sim.loc[mask, 'total_formal_products'] = X_sim.loc[mask, 'total_formal_products'] + 1
X_sim.loc[mask, 'total_products'] = X_sim.loc[mask, 'total_products'] + 1
X_sim.loc[mask, 'no_savings_no_access'] = 0
sim = get_vulnerability(X_sim)
interventions.append(("Formal Savings\\nAccess", baseline[mask].mean(), sim[mask].mean(), mask.sum(), (baseline[mask]>70).sum(), (sim[mask]>70).sum()))

# 4. Shock Protection
X_sim = X_all_i.copy()
mask = df_orig['experienced_shock'] == 'Yes'
X_sim.loc[mask, 'experienced_shock'] = 0
X_sim.loc[mask, 'shock_vulnerable'] = 0
X_sim.loc[mask, 'age_x_shock'] = 0
X_sim.loc[mask, 'shock_x_defaulted'] = 0
X_sim.loc[mask, 'shock_no_resilience'] = 0
X_sim.loc[mask, 'shock_x_no_savings'] = 0
X_sim.loc[mask, 'disability_x_shock'] = 0
sim = get_vulnerability(X_sim)
interventions.append(("Shock Protection\\n/ Insurance", baseline[mask].mean(), sim[mask].mean(), mask.sum(), (baseline[mask]>70).sum(), (sim[mask]>70).sum()))

# 5. Emergency Fund
X_sim = X_all_i.copy()
mask = df_orig['accessto_13k_1month'] == 'No'
X_sim.loc[mask, 'accessto_13k_1month'] = 1
X_sim.loc[mask, 'resilience_score'] = X_sim.loc[mask, 'resilience_score'] + 1
X_sim.loc[mask, 'shock_no_resilience'] = 0
sim = get_vulnerability(X_sim)
...
w = 0.35
bars1 = ax.bar(x - w/2, befores, w, label='Before', color='#e53935', alpha=0.85)
bars2 = ax.bar(x + w/2, afters, w, label='After', color='#43a047', alpha=0.85)
...
for name, before, after, count, hr_before, hr_after in interventions:
    print(f"  {name.replace(chr(10),' ')}: {before:.1f} -> {after:.1f} (delta={after-before:+.1f}), High-risk: {hr_before} -> {hr_after} ({hr_after-hr_before:+d})")""")
...
print(f'Mean: {vuln.mean():.1f}, High risk (>70): {(vuln>70).sum()} ({(vuln>70).mean()*100:.1f}%)')

print('\\n=== COUNTY RANKINGS (TOP 10 MOST VULNERABLE) ===')
cs = df.groupby('county').agg(n=('vuln','count'), avg=('vuln','mean'), worsened=('financial_status', lambda x: (x=='Worsened').mean())).sort_values('avg', ascending=False)
for c, r in cs.head(10).iterrows():
    print(f"  {c:20s} avg={r['avg']:.1f}  worsened={r['worsened']*100:.1f}%  n={int(r['n'])}")

for label in ['Worsened', 'Improved', 'Stayed the same']:
    subset = df[df['financial_status'] == label]
    print(f"\\n--- {label.upper()} PERSONA ---")
    print(f"  Age: {subset['Age'].mode().iloc[0]}, Sex: {subset['Sex'].mode().iloc[0]}")
    print(f"  Median income: KES {subset['monthly_income'].median():,.0f}")
    print(f"  Shock: {(subset['experienced_shock']=='Yes').mean()*100:.1f}%, Defaulted: {(subset['defaulted']=='Yes').mean()*100:.1f}%")
    print(f"  Formal savings: {(subset['Savings_formal']=='Usage').mean()*100:.1f}%, Can access 13k: {(subset['accessto_13k_1month']=='Yes').mean()*100:.1f}%")""")

md("## 9. Key Findings & Recommendations\n\n1. **Best Model:** CatBoost+Threshold (Weighted F1 ~0.5485)\n2. **#1 Driver:** Geographic location + economic shocks without safety nets\n3. **#1 Intervention:** Shock protection (insurance/safety nets) — rescues 1,104 from high-risk\n4. **#2 Intervention:** Emergency fund access (KES 13k within 30 days)\n5. **#3 Intervention:** Formal savings access\n6. **Paradox:** Mobile money expansion & financial literacy alone may increase vulnerability without structural resilience\n7. **Geographic targeting:** Tana River, Homabay, Kisumu need urgent intervention (FVI > 68)")
...
with open('odysseus_datasprint2026.ipynb', 'w', encoding='utf-8') as f:
